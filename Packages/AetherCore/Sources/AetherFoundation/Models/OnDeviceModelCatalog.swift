...
public enum DownloadSource: String, Codable, Sendable, CaseIterable {
...
public struct OnDeviceModelEntry: Identifiable, Sendable, Equatable {
    public let id: String                    // 模型 ID（用作本地目录名）
    public let name: String                  // 模型名
    public let description: String           // 中文简介
    public let estimatedSizeMB: Int          // 估计文件大小（MB）
    public let huggingFaceRepo: String       // HuggingFace 仓库 ID（如 "mlx-community/Llama-3.2-1B-Instruct-4bit"）
    public let modelScopeRepo: String?       // ModelScope 镜像仓库 ID（主地址失败时回退，nil 表示无镜像）
    public let sha256: String                // model.safetensors 的 SHA256 校验值
    ...
    public init(
        ...
        self.modelScopeRepo = modelScopeRepo
        self.sha256 = sha256
    }

    /// 根据下载源返回对应仓库 ID
    public func repo(for source: DownloadSource) -> String? {
        switch source {
...
public enum OnDeviceModelCatalog {
    /// 所有可用模型（均已验证 HuggingFace mlx-community 仓库可访问）
    public static let models: [OnDeviceModelEntry] = [
        OnDeviceModelEntry(
    ...
    /// 按 ID 查找模型
    public static func find(id: String) -> OnDeviceModelEntry? {
