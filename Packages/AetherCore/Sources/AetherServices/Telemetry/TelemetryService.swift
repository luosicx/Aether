...
public struct TelemetryRecord: Codable, Sendable, Equatable {
    /// 记录唯一标识
    public let id: UUID
    ...
    public init(id: UUID, event: String, payload: [String: String], timestamp: Date) {
        self.id = id
        self.event = event
...
public actor TelemetryService {
    ...
    public func drain() -> [TelemetryRecord] {
        let records = buffer
        buffer.removeAll()
        return records
    }
    ...
    public func shouldUpload(now: Date, threshold: Int = 100, interval: TimeInterval = 300) -> Bool {
        // 缓冲达到阈值即触发
