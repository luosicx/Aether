...
nonisolated public final class QwenClient: LLMProvider, @unchecked Sendable {
    /// URLSession 实例，lazy 加载默认用 .shared
    private lazy var session: URLSession = .shared
    /// SSE 流解析器
    private let parser = SSEParser()
    /// 当前 client 对应的供应商（取 baseURL / endpoints / models 的统一入口）
    private let provider: ModelProvider = .qwen
    ...
    private func sendRequest(body: ChatRequestBody, apiKey: String, continuation: AsyncStream<String>.Continuation) async {
        guard let url = URL(string: provider.baseURL + provider.chatEndpoint) else {
            continuation.finish()
            return
        }
    ...
    private func sendRequestWithTools(body: ChatRequestBody, apiKey: String, continuation: AsyncStream<ParsedChunk>.Continuation) async {
        ...
        guard let url = URL(string: provider.baseURL + provider.chatEndpoint),
