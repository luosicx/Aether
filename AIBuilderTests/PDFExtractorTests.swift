...
final class PDFExtractorTests: XCTestCase {
    ...
    private let fixtureText = "AIBuilder test PDF content for extraction. This is a unit test fixture."

    override func setUp() async throws {
        try await super.setUp()
        tempPDFURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("AIBuilderTest_\(UUID().uuidString).pdf")
        try generateTestPDF()
    }
    ...
    func testFixturePDFExtraction() {
        ...
        XCTAssertTrue(text?.contains("AIBuilder") ?? false, "提取的文本应含原文关键字 AIBuilder")
    }
